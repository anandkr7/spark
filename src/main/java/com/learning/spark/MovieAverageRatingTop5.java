...
public class MovieAverageRatingTop5 {
	...
	public static void main(String[] args) {
		...
		Dataset<Row> ratings = dataFrameReader.option("header", "true").csv("C:\\Users\\Anand\\eclipse-workspace\\spark\\src\\main\\resources\\in\\u.data");
		System.out.println("=== Print out schema ratings ===");
		ratings.printSchema();

		// Select the columns ItemID and Rating from the dataset. Also, cast the type of
		// Rating to double.

		Dataset<Row> ratingsLean = ratings.select(ratings.col("ItemID"),
				ratings.col("Rating").cast("double"));
		...
		Dataset<Row> ratingsAvgRename = ratingsAvg.select(ratingsAvg.col("ItemID"),
				ratingsAvg.col("avg(Rating)").as("Avg_Rating"));
		ratingsAvgRename.printSchema();

		// Sorting the records based on Avg_Rating in descending order and fetching top
		// 20 records using limit
		...
		Dataset<Row> joinedData = ratingsAvgSorted.join(movieDataLean,
				ratingsAvgSorted.col("ItemID").equalTo(movieDataLean.col("ItemID")), "inner");
		joinedData.printSchema();
		joinedData.show();

		// Fetching the movie name and Avg_Rating

		Dataset<Row> finalData = joinedData.select(joinedData.col("MovieName"), joinedData.col("Avg_Rating"))
				.orderBy(joinedData.col("Avg_Rating").desc());
